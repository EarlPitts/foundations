...
class ImperativeActionTest extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  test("onError gives back the result of action") {
  }

  test("onError calls cleanup") {
    var count = 0
    val cleanup = (_: Any) => count += 1
    val exception = new Exception("oh no")
    lazy val action = throw exception

    forAll { (n: Int) =>
      assert(Try(onError(n, cleanup)) == Success(n))
      assert(count == 0)
    }
