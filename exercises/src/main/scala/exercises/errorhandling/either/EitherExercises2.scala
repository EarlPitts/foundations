...
object EitherExercises2 {
  ...
  def checkUsernameSize(username: String): Either[TooSmall, Unit] = {
    ...
    Either.cond(len >= 5, (), TooSmall(len))
  }
  ...
  def checkUsernameCharacters(username: String): Either[InvalidCharacters, Unit] = {
    val invalid = username.filterNot(isValidUsernameCharacter)
    Either.cond(invalid.isEmpty(), (), InvalidCharacters(invalid.toList))
  }
  ...
  def validateUsername(username: String): Either[UsernameError, Username] = for {
  ...
  def validateUser(usernameStr: String, countryStr: String): Either[UserCreationError, User] = for {
    username <- validateUsername(usernameStr)
    country <- validateCountry(countryStr)
  } yield User(username = username, country)

  sealed trait UserCreationError

  sealed trait CountryError extends UserCreationError
  object CountryError {
    case class InvalidFormat(country: String) extends CountryError
    case class NotSupported(country: String)  extends CountryError
  }

  sealed trait UsernameError extends UserCreationError
