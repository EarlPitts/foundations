...
object OptionExercises {
  ...
  sealed trait Role {
    ...
    def asEditor: Option[Editor] = this match {
  ...
  def sequence[A](options: List[Option[A]]): Option[List[A]] =
    options.foldRight(Option(List.empty[A])) { (curr, acc) =>
      curr.zip(acc).map { case (head, tail) => head :: tail }
    }
  ...
  // Note: Once you have implemented `getAccountId`, try to move it
  //       inside the `Role` class.
  def asEditor(role: Role): Option[Editor] = role match {
    case x: Editor => Some(x)
    case _ => None
  }
}
