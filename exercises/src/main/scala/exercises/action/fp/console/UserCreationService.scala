...
class UserCreationService(console: Console, clock: Clock) {
  ...
  val readDateOfBirth: IO[LocalDate] = {
    val printError = console.writeLine("""Incorrect format, for example enter "18-03-2001" for 18th of March 2001""")
    for {
      _    <- console.writeLine("What's your date of birth? [dd-mm-yyyy]")
      line <- console.readLine
      answer <- parseDateOfBirth(line)
        .onError(_ => printError)
    } yield answer
  }
  ...
  val readSubscribeToMailingList: IO[Boolean] = {
    val printError = console.writeLine("""Incorrect format, enter "Y" for Yes or "N" for "No"""")
    for {
